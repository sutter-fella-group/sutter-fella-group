...
elseif NumberOfPlots==6
    ...
    for m=1:SecOne
        ColorSet(m,:)=[0 0 (m-1)/(SecOne-1)];
    end

    for m=1:SecTwo
        ColorSet(m+SecOne,:)=[0 m/2/(SecTwo) 1];
    end
    
    for m=1:SecThree
        ColorSet(m+SecOne+SecTwo,:)=[(m)/(SecThree) 0.5 1];
    end
    
    for m=1:SecFour
        ColorSet(m+SecOne+SecTwo+SecThree,:)=[1 (SecFour-m)/2/(SecFour) (SecFour-m)/(SecFour)];
    end
